...
weightcenterentropy_log_shave<-function(x,ntoshave=7,delta)
{#x is a matrix. 
  ...
  # total output diminsion is ntoshave+1 columns 
  ...
  { # caculate the WH for different conditions.
    ...
    outdat<-outent*(1-pH)+outent1*pH #Tukey biweight?
    outmat[,i]<-outdat
  }
...
weightcenterentropy_log_shave_identify<-function(x,thre,delta)
{#x is a matrix. 
